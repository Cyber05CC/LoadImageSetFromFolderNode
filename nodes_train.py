...
def _resolve_dataset_folder(dataset_folder):
    dataset_folder = os.path.expanduser(str(dataset_folder).strip().strip('"'))
    if os.path.isabs(dataset_folder) and os.path.isdir(dataset_folder):
        return dataset_folder

    candidates = [
        os.path.join(folder_paths.get_input_directory(), dataset_folder),
        os.path.join(folder_paths.get_output_directory(), dataset_folder),
    ...
    raise FileNotFoundError(
        f"Dataset folder not found: {dataset_folder}. Try an absolute path or put it in ComfyUI/input."
    )


def _list_dataset_images(dataset_dir, recursive):
    walker = os.walk(dataset_dir) if recursive else [(dataset_dir, [], os.listdir(dataset_dir))]
    image_paths = []
    for root, _, files in walker:
        for filename in files:
            if os.path.splitext(filename)[1].lower() in IMAGE_EXTENSIONS:
                image_paths.append(os.path.join(root, filename))
    image_paths.sort(key=lambda path: os.path.relpath(path, dataset_dir).lower())
    if not image_paths:
        raise ValueError(f"No images found in dataset folder: {dataset_dir}")
    return image_paths


def _read_caption_for_image(image_path, default_caption, require_captions):
    root, _ = os.path.splitext(image_path)
    caption_candidates = [
        root + ".txt",
        image_path + ".txt",
        os.path.join(os.path.dirname(image_path), "caption.txt"),
    ]
    ...
    if require_captions:
        raise FileNotFoundError(
            f"Missing caption for {os.path.basename(image_path)}. Expected {os.path.basename(root)}.txt."
        )
    return default_caption.strip()


def _resize_dataset_image(image, width, height, resize_mode):
...
def _image_to_tensor(image_path, width, height, resize_mode):
    image = Image.open(image_path)
    image = ImageOps.exif_transpose(image).convert("RGB")
    image = _resize_dataset_image(image, width, height, resize_mode)
    array = np.asarray(image).astype(np.float32) / 255.0
    return torch.from_numpy(array)


def _encode_caption_list(clip, captions):
    conditioning = []
...
class LoadImageSetFromFolderNode(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="LoadImageSetFromFolderNode",
            search_aliases=["dataset captions", "load image dataset", "load folder captions"],
            display_name="Load Image Set From Folder",
            category="image/training",
            is_experimental=True,
            inputs=[
                io.Custom("CLIP").Input(
                    "clip",
                    tooltip="CLIP model used to encode each caption text file.",
                ),
                io.String.Input(
                    "dataset_folder",
                    default="my_dataset",
                    tooltip="Folder path. Relative paths are searched in ComfyUI/input first.",
                ),
                io.Combo.Input(
                    "resize_mode",
                    options=["Stretch", "Center Crop", "Pad", "Keep"],
                    default="Stretch",
                    tooltip="How images are resized before VAE encoding.",
                ),
                io.Int.Input(
                    "width",
                    default=1024,
                    ...
                    max=4096,
                    step=8,
                    tooltip="Output image width. Ignored when resize_mode is Keep.",
                ),
                io.Int.Input(
                    "height",
                    default=1024,
                    ...
                    max=4096,
                    step=8,
                    tooltip="Output image height. Ignored when resize_mode is Keep.",
                ),
                io.Boolean.Input(
                    "recursive",
                    default=True,
                    tooltip="Read images from subfolders too.",
                ),
                io.Boolean.Input(
                    "require_captions",
                    default=True,
                    tooltip="Fail if an image has no matching caption text file.",
                ),
                io.String.Input(
                    "default_caption",
                    default="",
                    multiline=True,
                    tooltip="Fallback caption when require_captions is disabled.",
                ),
            ],
            outputs=[
                io.Image.Output(display_name="IMAGE", tooltip="Batch of loaded dataset images."),
                io.Conditioning.Output(
                    display_name="CONDITIONING",
                    tooltip="One encoded caption per image, in the same order as the image batch.",
                ),
                io.Int.Output(display_name="count", tooltip="Number of images loaded."),
            ],
        )
    ...
    def execute(
        ...
        dataset_dir = _resolve_dataset_folder(dataset_folder)
        image_paths = _list_dataset_images(dataset_dir, recursive)
        tensors = [
            _image_to_tensor(image_path, width, height, resize_mode)
        ...
        if len(shapes) != 1:
            raise ValueError(
                "All images must have the same size for one IMAGE batch. Use Stretch, Center Crop, or Pad."
            )

        captions = [
            _read_caption_for_image(image_path, default_caption, require_captions)
            for image_path in image_paths
        ]
        conditioning = _encode_caption_list(clip, captions)
        if len(conditioning) != len(tensors):
            logging.warning(
                "Caption encoding count (%s) differs from image count (%s).",
                len(conditioning),
                len(tensors),
            )

        images = torch.stack(tensors, dim=0)
        logging.info("Loaded dataset %s: %s images", dataset_dir, len(tensors))
        return io.NodeOutput(images, conditioning, len(tensors))


class LoadImageSetFromFolderLegacy:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    RETURN_TYPES = ("IMAGE", "CONDITIONING", "INT")
    RETURN_NAMES = ("IMAGE", "CONDITIONING", "count")
    ...
    def load_dataset(
        ...
        images = torch.stack(tensors, dim=0)
        logging.info("Loaded dataset %s: %s images", dataset_dir, len(tensors))
        return images, conditioning, len(tensors)
        ...
        for i in (
            ...
            if (i + 1) % self.grad_acc == 0:
                ...
                if self.checkpoint_callback:
                    self.checkpoint_callback((i + 1) // self.grad_acc)
            ui_pbar.update(1)
        torch.cuda.empty_cache()
...
class TrainLoraNode(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    ...
                    min=0,
                    max=1000000,
                    tooltip="Save an intermediate LoRA safetensors file every N optimizer steps. Set 0 to disable.",
                ),
                io.String.Input(
                    "checkpoint_prefix",
                    default="runninghub_lora/checkpoints/my_zimage_lora",
                    tooltip="Output prefix for intermediate LoRA checkpoints.",
                ),
            ],
            outputs=[
    ...
    def execute(
        ...
        checkpoint_prefix="runninghub_lora/checkpoints/my_zimage_lora",
    ):
        ...
        checkpoint_every_steps = checkpoint_every_steps[0] if isinstance(checkpoint_every_steps, list) else checkpoint_every_steps
        checkpoint_prefix = checkpoint_prefix[0] if isinstance(checkpoint_prefix, list) else checkpoint_prefix
        ...
        with torch.inference_mode(False):
            ...
            def save_checkpoint_callback(current_step):
                if checkpoint_every_steps <= 0 or current_step % checkpoint_every_steps != 0:
                    return
                ...
                checkpoint_path = os.path.join(
                    full_output_folder,
                    f"{filename}_{current_step}_steps_{counter:05}.safetensors",
                )
                checkpoint_sd = {
                    key: value.detach().to(lora_dtype).cpu()
                    for key, value in lora_sd.items()
                }
                safetensors.torch.save_file(checkpoint_sd, checkpoint_path)
                logging.info("Saved intermediate LoRA checkpoint: %s", checkpoint_path)
...
class TrainLoraCheckpointedNode:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "steps": ("INT", {"default": 3000, "min": 1, "max": 1000000}),
                "learning_rate": ("FLOAT", {"default": 0.0001, "min": 0.0, "max": 1.0, "step": 0.000001}),
                "rank": ("INT", {"default": 32, "min": 1, "max": 128}),
                ...
                "training_dtype": (["bf16", "fp32", "none"], {"default": "bf16"}),
                "lora_dtype": (["bf16", "fp32"], {"default": "bf16"}),
                "quantized_backward": ("BOOLEAN", {"default": False}),
                "algorithm": (list(adapter_maps.keys()), {"default": list(adapter_maps.keys())[0]}),
                "gradient_checkpointing": ("BOOLEAN", {"default": True}),
                "checkpoint_depth": ("INT", {"default": 1, "min": 1, "max": 5}),
                "offloading": ("BOOLEAN", {"default": True}),
                ...
                "checkpoint_every_steps": ("INT", {"default": 1000, "min": 0, "max": 1000000}),
                "checkpoint_prefix": ("STRING", {"default": "runninghub_lora/checkpoints/my_zimage_lora"}),
            }
        }

    RETURN_TYPES = ("LORA_MODEL", "LOSS_MAP", "INT")
